...
public:
    ...
    double defaultPolicy(Node * node,int ALPlay); //以node为初始结点，开始随机模拟，并返回reward
    void backUp(Node * node,double reward); //从模拟初始结点开始一直回溯到根结点，更新N值和Q值
    //辅助函数
    ...
    double quickEvaluate(Node * node,int AIPlay); //对当前node棋面打分
};
